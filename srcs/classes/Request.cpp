...
Request::Request(int clientId, sockaddr_in client_addr, const Config &configuration)
    : clientId((size_t)clientId), client_addr(client_addr), configuration(configuration) {}
...
bool Request::readRequest(int clientSd, Response &res)
{
    ...
    if (bodySize == 0)
        return true;
    if(bodySize > configuration.getMaxBody())
    {
